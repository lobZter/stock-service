(function($) {
  ...
  function setStockOption() {
    ...
    $.get(url, function(data, status) {
      ...
      while(i < Object.keys(stocks).length) {
        var val = "{'stock_class':'"+stocks[i].stock_class+"','date_issued':'"+stocks[i].date_issued+"'}";
        var show = stocks[i].stock_class + ' / ' + stocks[i].date_issued;
  ...
  $(function() {
    ...
    $('#report_datetimepicker1').datetimepicker({
       format: 'YYYY/MM/DD'
    })
    
    $('#report_datetimepicker2').datetimepicker({
       format: 'YYYY/MM/DD'
    })
